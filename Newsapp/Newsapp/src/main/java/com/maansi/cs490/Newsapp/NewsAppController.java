...
public class NewsAppController {
	...
	public @ResponseBody Iterable<tbl_signup> getAllUsers() {
	  ...
	  public @ResponseBody String newPassword(@RequestParam String password, @RequestParam String username) {
		String p = userRepository.getPassword(username);
		 if(p == null || p.isEmpty()) {
			 return "Invalid username";
		 }
		 if( p.compareTo(password) == 0) {
			 return "You cannot use same password";
		 } else {
